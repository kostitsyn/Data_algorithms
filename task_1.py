...
'''Версия Python 64-х разрядная
Вывод: меньше всего памяти расходует вариант 2, поскольку в качестве переменных в нём используются только один
массива array и две переменных i и max_neg_num, а поскольку эти переменные ссылаются на значения, которые уже
есть в массиве, то затраты памяти составляют только объём самого самого массива плюс объём неповторяющихся
